...
def train_fold(fold, args, splits_base_dir, logger, dry_run=False):
    ...
    cmd = [
        ...
        '--max-prediction-length', str(args.max_prediction_length),
        # Training
...
def aggregate_results(folds, args, logger):
    ...
    for metric in key_metrics:
        if metric in df.columns:
            mean = df[metric].mean()
            std = df[metric].std()
            logger.info(f"  {metric:<25}: {mean:>8.4f} Ã‚Â± {std:.4f}")
...
def parse_args():
    ...
    parser.add_argument('--quantiles', type=str, default='7q',
                        choices=['3q', '5q', '7q'],
                        help='Quantile configuration (3q, 5q, or 7q)')
